...
import { BoardLayoutProvider, BoardLayoutConstants, useBoardLayoutContext } from "./BoardLayoutProvider";
import { BoardDataProvider, BoardDataState, useBoardDataContext } from "./BoardDataProvider";
...
const Panel: React.FC<React.PropsWithChildren<IPanelProps>> = ({ className, children }) => {
  ...
  const boardDataContextRef = React.useRef(boradDataState);
  const groupIndicateRef = React.useRef<null | IGroupIndicate>(null);
...
const Groups = React.forwardRef<React.ElementRef<"div">, IGroupsProps>(({ children, className, ...props }, forwardedRef) => {
  ...
  return (
    <div ref={forwardedRef} className={className} {...props}>
      {groupIds.map((groupId) => React.cloneElement(children as React.ReactElement<{ groupData: IGroup[keyof IGroup] }>, { key: groupId, groupData: boradDataState.group[groupId] }))}
    </div>
  );
});
...
const Group = React.forwardRef<React.ElementRef<"div">, IGroupProps>(({ children, className, groupData, mdxSources, ...props }, forwardedRef) => {
  ...
  return (
    <div
      ...
      {...props}
    >
      {React.Children.map(children, (child) => (React.isValidElement(child) ? React.cloneElement(child as React.ReactElement<IGroupProps>, { groupData, mdxSources }) : child))}
      ...
      {groupData.tabIds.map((tabId, idx) =>
        React.Children.map(children, (child) =>
          React.isValidElement(child)
            ? React.cloneElement(child as React.ReactElement<{ groupId: string; tabId: string; tabIdx: number; mdxSources?: { [key: string]: any }; groupData?: IGroup[keyof IGroup] }>, {
                key: tabId,
                groupId: groupData.id,
                tabId,
                tabIdx: idx,
...
const TabContent = React.forwardRef<React.ElementRef<"div">, ITabContentProps>(({ children, className, groupData, mdxSources }, forwardedRef) => {
  ...
  return (
    <div ref={forwardedRef} className={cn("overflow-auto", className)}>
      {!mdxContent ? <div>{children}</div> : React.cloneElement(children as React.ReactElement<{ groupId: string; mdxContent: any }>, { groupId: id, mdxContent: mdxContent })}
    </div>
  );
});
...
const ResizeHandlers = ({ groupId }: { groupId: string }) => {
  const resizeHandlerElementPosition = React.useMemo(() => ({ x: 0, y: 0 }), []);
